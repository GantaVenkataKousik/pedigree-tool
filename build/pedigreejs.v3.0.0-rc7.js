var pedigreejs = (function (exports) {
	...
	**/
	...
	function openEditDialog(d) {
	  ...
	  table += "<tr><td style='text-align:right'>Unique ID</td><td><input class='form-control' type='text' id='id_name' name='name' value='" + (d.data.name || "") + "'></td></tr>";
	  table += "<tr><td style='text-align:right'>Name</td><td><input class='form-control' type='text' id='id_display_name' name='display_name' value='" + (d.data.display_name || "") + "'></td></tr>";
	  table += "<tr><td style='text-align:right'>Age</td><td><input class='form-control' type='number' id='id_age' min='0' max='120' name='age' style='width:7em' value='" + (d.data.age || "") + "'></td></tr>";
	  table += "<tr><td style='text-align:right'>Year Of Birth</td><td><input class='form-control' type='number' id='id_yob' min='1900' max='2050' name='yob' style='width:7em' value='" + (d.data.yob || "") + "'></td></tr>";

	  // Radio buttons for gender
	  table += '<tr><td colspan="2" id="id_sex">' + '<label class="radio-inline"><input type="radio" name="sex" value="M" ' + (d.data.sex === 'M' ? "checked" : "") + '> Male</label>' + '<label class="radio-inline"><input type="radio" name="sex" value="F" ' + (d.data.sex === 'F' ? "checked" : "") + '> Female</label>' + '<label class="radio-inline"><input type="radio" name="sex" value="U" ' + (d.data.sex === 'U' ? "checked" : "") + '> Unknown</label>' + '</td></tr>';
	  // Radio buttons for status
	  table += '<tr><td colspan="2" id="id_status">' + '<label class="radio-inline"><input type="radio" name="status" value="0" ' + (d.data.status == "0" ? "checked" : "") + '> Alive</label>' + '<label class="radio-inline"><input type="radio" name="status" value="1" ' + (d.data.status == "1" ? "checked" : "") + '> Deceased</label>' + '</td></tr>';
	  // Pregnant checkbox (if applicable)
	  table += '<tr><td colspan="2" id="id_pregnant">' + '<label class="checkbox-inline"><input type="checkbox" id="pregnant_checkbox" ' + (d.data.pregnant ? "checked" : "") + '> Pregnant</label>' + '</td></tr>';
	  ...
	  for (let attr of switches) {
	    table += '<tr><td colspan="2">' + '<label class="checkbox-inline"><input type="checkbox" id="id_' + attr + '" ' + (d.data[attr] ? "checked" : "") + '> ' + attr.replace('_', ' ').replace(/^\w/, c => c.toUpperCase()) + '</label></td></tr>';
	  }
	...
	<defs>
	  ...
	  for (let i = 0; i < opts.dataset.length; i++) {
	    ...
	    let $personBox = $('<div></div>').addClass('person-box').css({
	      width: opts.symbol_size + 'px',
	      height: opts.symbol_size + 'px',
	      backgroundColor: opts.node_background,
	      // default background
	    ...
	    for (let disease of opts.diseases) {
	      if (person[disease.type]) {
	        // If person has this disease
	...
	**/
	...
	function save(opts) {
	  ...
	  update_ashkn(newdataset);
	  if ($('#id_approx').is(':checked')) person.approx_diagnosis_age = true;else delete person.approx_diagnosis_age;
